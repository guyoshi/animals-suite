...
export function DashboardPage() {
  ...
  return <div>
    <PageHeader title="Status geral" subtitle="Produção, cobertura, mapas e próximos passos do projeto." actions={<button className="primary-button" onClick={()=>navigate('/executor/production')}><Wrench/> Abrir Produção</button>}/>
    <div className="dashboard-primary-kpis">
      <button onClick={()=>navigate('/executor/production')}><CircleDashed/><div><strong>{active.length}</strong><span>Elementos ativos</span></div></button>
      <button onClick={()=>navigate('/executor/production?filter=unity')}><CheckCircle2/><div><strong>{active.filter(r=>r.status==='unity').length}</strong><span>Configurados no Unity</span></div></button>
      <button onClick={()=>navigate('/executor/production?filter=unplaced')}><MapPinOff/><div><strong>{active.filter(r=>!r.placed).length}</strong><span>Ainda fora do mapa</span></div></button>
      <button onClick={()=>navigate('/executor/production?filter=no-relations')}><Link2Off/><div><strong>{active.filter(r=>r.relationCount===0).length}</strong><span>Sem relações</span></div></button>
      <button onClick={()=>navigate('/executor/production?filter=errors')} className="danger-kpi"><AlertTriangle/><div><strong>{active.filter(r=>r.status==='erro').length}</strong><span>Com erro no Unity</span></div></button>
    </div>
    ...
    <div className="dashboard-grid">
      ...
      <Card><SectionTitle>Mundos</SectionTitle><div className="world-card-grid">{project.worlds.map(world=>{const areas=project.areas.filter(a=>a.worldId===world.id);const worldRows=active.filter(r=>r.worldId===world.id);const placed=worldRows.filter(r=>r.placed).length;const errors=worldRows.filter(r=>r.status==='erro').length;return <button className="world-mini-card" style={{'--world':world.theme.primary} as React.CSSProperties} key={world.id} onClick={()=>navigate(`/executor/production?world=${world.id}`)}><span className="world-swatch"/><div><strong>{world.name}</strong><small>{areas.length} áreas · {placed}/{worldRows.length} elementos no mapa · {errors} erros</small></div></button>})}</div></Card>
    </div>
